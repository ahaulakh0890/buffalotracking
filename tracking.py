...
# ================= USER INPUT =================
tracking_number = input("➡️ Tracking number enter karo: ").strip()


if not tracking_number:
    print("❌ Tracking number empty hai")
    exit()
...
tracking_input = wait.until(
    EC.presence_of_element_located((By.ID, "order-no"))
)
...
sign_status = driver.find_element(
    By.CSS_SELECTOR,
    "#result-content div.record-data"
).text.strip()
...
records = driver.find_elements(
    By.XPATH,
    "//div[@id='result-content']//div[contains(@class,'record-data')]"
)
...
print(f"\n📄 PDF saved successfully: {pdf_name}")
print("✅ DONE")